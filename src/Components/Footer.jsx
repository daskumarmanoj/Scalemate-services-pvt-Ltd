...
/* ─────────────────────────────────────────────
   ...
   lucide-react does NOT export: Facebook, Instagram,
   Twitter, Youtube, Linkedin — using raw SVGs instead
─────────────────────────────────────────────*/
...
─────────────────────────────────────────────*/
const COMPANY = {
  whatsapp: "916371287364",
  phone: "+91 98765 43210",
  email: "hello@scalemate.in",
  address: "Bengaluru, Karnataka — Pan India",
  description:
    "India's trusted business growth partner. From incorporation to IPO — we handle the complexity so you can focus on building.",
};

const LINKS = [
  {
    heading: "Services",
    items: ["Company Registration","Legal Compliance","Funding & Loans","Intellectual Property","HR & Payroll","Accounting Services"],
  },
  {
    heading: "Grow",
    items: ["Business Consulting","Growth Strategy","Import Export","Startup Support","GST Filing","MSME Registration"],
  },
  {
    heading: "Company",
    items: ["About Us","Our Team","Careers","Blog","Press Kit","Contact"],
  },
];
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            {items.map((item, i) => (
              <li key={i}>
                <a href="#" className="block py-1.5 text-[#666] text-sm hover:text-amber-400 transition-colors duration-200 no-underline">
                  {item}
                </a>
              </li>
            ))}
...
function ContactRow({ icon: Icon, value, href }) {
  return (
    <a href={href} className="flex items-center gap-2.5 text-[#666] hover:text-amber-400 transition-colors duration-200 no-underline text-sm group">
  ...
  return (
    <footer className="bg-[#080808] text-white relative overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: "radial-gradient(rgba(250,204,21,0.04) 1px, transparent 1px)", backgroundSize: "28px 28px" }}
      />
      ...
      <div className="max-w-275 mx-auto px-5 relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 border-b border-white/[0.07]">
          {STATS.map((s, i) => (
            <motion.div
              ...
              className={`py-7 px-3 text-center ${i % 2 === 0 ? "border-r border-white/[0.07]" : ""} sm:nth-2:border-r sm:nth-3:border-r sm:nth-4:border-r-0`}
            >
              <div className="text-3xl sm:text-4xl font-black text-amber-400 leading-none tracking-tight">
                {s.value}
              </div>
              <div className="text-xs text-[#555] mt-1.5 font-medium tracking-wide">{s.label}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1fr] gap-10 lg:gap-12 py-14">
          ...
          >
            ...
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-[0_4px_16px_rgba(250,204,21,0.3)]"
                style={{ background: "linear-gradient(135deg,#facc15,#f59e0b)" }}>
                <Sparkles size={19} color="#080808" strokeWidth={2.2} />
              </div>
              <div>
                <div className="text-xl font-black text-white leading-none tracking-tight">Scalemate</div>
                <div className="text-[10px] text-[#555] tracking-widest mt-0.5">SERVICES PVT. LTD.</div>
              </div>
            </div>

            <p className="text-[#555] text-sm leading-relaxed mb-5 max-w-xs">{COMPANY.description}</p>
            ...
            {/* WhatsApp pill */}
            <motion.a
              href={`https://wa.me/${COMPANY.whatsapp}?text=${encodeURIComponent("Hello Scalemate! I'd like to know more.")}`}
            ...
            <div className="flex gap-2 flex-wrap">
              {SOCIALS.map(({ Icon, label, color, hoverBg, hoverBorder }, i) => (
                <motion.a
                  key={i} href="#" aria-label={label}
                  whileHover={{ y: -3 }} transition={{ duration: 0.18 }}
                  className="w-9 h-9 rounded-xl flex items-center justify-center no-underline border border-white/9 bg-white/5 transition-all duration-200"
          ...
          {LINKS.map((col, ci) => (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-black text-white tracking-widest uppercase">{col.heading}</span>
              </div>
              <ul className="list-none p-0 m-0">
                {col.items.map((item, ii) => (
                  <li key={ii}>
                    <a href="#" className="group flex items-center gap-1.5 text-[#555] text-sm hover:text-amber-400 transition-colors duration-200 no-underline py-1">
                      <ArrowRight
                        size={11} color="#facc15"
          ...
          <div className="md:hidden col-span-1">
            {LINKS.map((col, i) => <Accordion key={i} heading={col.heading} items={col.items} />)}
          </div>
        </div>
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 bg-amber-400/5 border border-amber-400/[0.14] rounded-2xl px-6 py-5 mb-8"
        >
          <div>
            <div className="text-base font-extrabold text-white mb-1">Business tips in your inbox 📬</div>
            <div className="text-xs text-[#555]">Weekly insights on compliance, funding & growth. No spam.</div>
          </div>

          <div className="w-full sm:w-auto sm:min-w-75 shrink-0">
            <AnimatePresence mode="wait">
              {subStatus === "success" ? (
                ...
                <motion.div key="form" className="flex flex-col sm:flex-row gap-2 sm:gap-0">
                  <input
                    type="email" placeholder="your@email.com" value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && handleSubscribe()}
                    className="flex-1 bg-white/5 border border-white/12 sm:border-r-0 rounded-xl sm:rounded-r-none px-3.5 py-2.5 text-white text-sm placeholder-[#555] outline-none focus:border-amber-400/40 focus:bg-white/8 transition-all duration-200"
                  />
                  <button
                    onClick={handleSubscribe}
                    className="flex items-center justify-center gap-1.5 px-4 py-2.5 font-bold text-sm text-[#080808] rounded-xl sm:rounded-l-none cursor-pointer border-none hover:opacity-90 transition-opacity whitespace-nowrap"
                    style={{ background: "linear-gradient(135deg,#facc15,#f59e0b)" }}
                  >
        ...
        <div className="border-t border-white/[0.07] py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="text-xs text-[#3a3a3a] leading-relaxed">
            ...
            <br />
            <span className="text-[#2e2e2e]">CIN: U74999KA2024PTC000001 · GST: 29AABCS1234N1Z5</span>
          </div>

          <div className="flex flex-wrap justify-center gap-4 sm:gap-5">
            {LEGAL.map((l, i) => (
              <a key={i} href="#" className="text-[#444] text-xs hover:text-amber-400 transition-colors duration-200 no-underline">{l}</a>
            ))}
          </div>
